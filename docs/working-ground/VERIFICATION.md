...
| R12 fixed-dimensional selection (in progress) | **Written proof:** polynomial-bit decision/order output at fixed affine dimension; two scoped sharpness refinements | 14 polygon/segment/point fixtures versus 84 exhaustive full orders using independent cvc5 encoding; rational witness checks and CPC proof skeletons | none; solver-trusted emptiness replay only | final replay audit pending; no external CPC kernel; R08 strategic bridge remains unformalized; growing dimension unresolved |
