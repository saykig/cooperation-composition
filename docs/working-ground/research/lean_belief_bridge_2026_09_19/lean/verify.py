"""Compile fresh R14 objects against pinned Mathlib; audit every local declaration.

The cache is read-only. No previously built R14 object is reused. The receipt
binds each source and audit declaration to its SHA-256 and compiler dependencies.
"""
import argparse
import hashlib
import json
...
def run(args, env=None):
    result = subprocess.run(args, env=env, text=True, capture_output=True, timeout=600)
...
def main():
    ...
    declarations = []
    logs = {}
    with tempfile.TemporaryDirectory(prefix='ce-r14-proof-') as temp:
        build = Path(temp)
        ...
        for module in args.modules:
            ...
            names = re.findall(r'^(?:def|abbrev|lemma|theorem)\s+(\w+)', content, re.M)
            if not names:
        ...
        if set(audited) != set(declarations):
            raise SystemExit('Audit missing declarations:\n' + output)
    ...
    receipt = dict(status='Lean checked; formal scope only', toolchain=version,
                   ...
                   modules=args.modules, audited_declarations=audited,
                   audit_sha256=hashlib.sha256(audit.encode()).hexdigest(), compiler_output=logs)
    rendered = json.dumps(receipt, indent=2, sort_keys=True) + '\n'
    if args.output:
        if args.output.exists():
            raise SystemExit('Refusing to overwrite an earlier proof receipt')
