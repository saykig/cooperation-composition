...
r=subprocess.run(cmd+[str(root/'JointFeasibility.lean')],env=dict(os.environ,LEAN_PATH=':'.join(paths)),text=True,capture_output=True)
log=(r.stdout+r.stderr).replace(str(root)+'/', '')
...
print(json.dumps({'status':'checked','toolchain':version,'mathlib_revision':rev,
  'source_sha256':hashlib.sha256((root/'JointFeasibility.lean').read_bytes()).hexdigest(),
  'output':log},indent=2))
